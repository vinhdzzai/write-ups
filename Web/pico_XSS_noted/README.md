...
3. Truy cập vào URL do ta truyền vào (không bị filter giao thức).
...
- Ở trên ta xác nhận /notes xảy ra XSS và /report để khiến bot fetch đến URL , và trang web thì actives theo user.id nên việc truyền url http://saturn.picoctf.net:49828/notes để bot thấy payload do attacker tạo ra là không thể vì session 2 bên khác nhau
---
...
1. Khiến bot mở một tab mới đặt tên là `"pico"` và truy cập vào trang `/notes` của chính nó (tab này chứa Flag và đúng Origin).
2. Khiến tab ban đầu của bot thực hiện đăng nhập (`POST /login`) vào tài khoản của Hacker (tài khoản này đã được tạo sẵn một Note chứa mã độc XSS).
...
<details>
<summary><b>Browsing group context</b></summary>

Kịch bản mở tab Bot (pico), đổi session sang Hacker rồi chạy XSS để dùng `window.open('', 'pico')` đọc ngược DOM hoạt động hoàn hảo trên Bot Chrome Headless (CTF/Lab) vì trình duyệt giả lập thường tắt cơ chế cô lập (`--disable-site-isolation`), cho phép các tab Cùng Origin + Biết Tên tự do chiếm quyền nhau. Tuy nhiên, trên trình duyệt thực tế, kỹ thuật này sẽ thất bại hoàn toàn do cơ chế Strict Site Isolation và Browsing Context Group (BCG) lập tức cô lập tiến trình và xóa bỏ liên kết tìm kiếm theo tên nếu không có mối quan hệ bộ nhớ (opener/reference) trực tiếp.
</details>

---
`Do trình độ của tôi có hạn nên tôi chỉ xây dựng được payload XSS còn payload cho /notes thì hơi khoai 😭 nên đành nhờ sự trợ giúp từ người thân`
